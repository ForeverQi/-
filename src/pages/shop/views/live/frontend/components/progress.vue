  ...
  <div class="container">
    <div class="progress" v-if="gressType == 1">
      <div class="progress_left" @click="playStatus">
        <img
          v-show="play == 0 && videoProcessEnd == 0"
          src="@/pages/shop/views/live/frontend/images/video_pause.png"
          alt
        />
        <img
          v-show="play == 1 || videoProcessEnd == 1"
          src="@/pages/shop/views/live/frontend/images/video_play.png"
          alt
        />
      </div>
      <div class="progress_center" id="progress_center">
      <!-- <div class="progress_center" id="progress_center" @click="proClick"> -->
        ...
        <van-slider v-model="videoProcess2" @change="onChange" />
      </div>
...
export default {
  ...
  data() {
    return {
      play: 0,
      value: 20,
      videoProcess2: this.videoProcess
    };
  },
  components: {},
  mounted() {
  ...
  methods: {
    ...
    proClick() {
      let that = this;
      let progress_center = document.getElementById("progress_center")
        .offsetLeft;
      ...
      // 进度条总长度
      let lengthAll = Number(
        document.getElementById("progress_center").offsetWidth
      );
      // 点击位置距离总进度条左边的距离
      ...
      this.$emit(
        "proClick",
        Number((video.currentTime / video.duration) * 100)
      );
    },
    // 拖动进度条
    dragFun() {
      let that = this;
      let progress_center = Number(
        document.getElementById("progress_center").offsetLeft
      );
      document
      ...
      $("#progress_btn").on("touchmove", function(e) {
        e.preventDefault();
        var moveEndX =
          Number(e.originalEvent.changedTouches[0].pageX) - progress_center;
        let video = document.querySelectorAll("video")[0];
  ...
  watch: {
      videoProcess(val,oldVal){
          this.videoProcess2 = parseInt(val)
      }
  }
};
</script>
