        ...
        <div class="layer-con-wrap" v-if="Layer.conShow">
            <div class="list-layout add-item-wrap">
                ...
                <div class="list-layout-b">
                    <div class="search-box" v-if="Link.list.length && 0">
                    ...
                    <div class="list-box">
                        ...
                        <ul v-if="!Link.loading && Link.list.length">
                            <li v-for="(item, index) in Link.list" :key="index">
                                <div class="text-box">{{item.classname}}</div>
                                <div class="btn-box">
                                    <button class="default-btn" v-if="item.ok_down" @click="getTwoLevelData(item)">下一级</button>
...
export default {
    ...
    methods: {
        ...
        getTwoLevelData(item){
            ...
            if(item.type == 1){
                Link.titleText = item.classname;
            }else{
                Link.titleText += ' >> '+item.classname;
            }
            productClass(postData).then(res => {
                let list = Link.list,
                    linkData = res;
                list.length = 0;
                if(Array.isArray(linkData)){
                    linkData.forEach(v => {
                        v.type = postData.type;
                        list.push(v);
                    });
                }
                Link.loading = false;
