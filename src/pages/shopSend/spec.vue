<template>
  <div class="send_warper">
    <div v-if="!showDeatil" class="list">
      <div @click="todetail(item.id)" v-for=" (item,index) in list" :key="item.id" class="_row flex_rowBetwen">
        <span>{{item.name}}</span>
    ...
    <div v-if="showDeatil" class="detail">
      <div class="send_warper">
        <div v-for=" (item,index) in list" :key="item.id" class="_rowBox">
          <div @click="change(index,'select') " class="_row paddl_r flex_rowBetwen">
            <span class="flex_center" @click.stop="change(index,'checkBox')">
              <van-checkbox shape="square" icon-size="14"  v-model="item.checkBox">{{item.name}}</van-checkbox>
            </span>
            <span>
        ...
        <div v-if="list.length == 0" class="flex_center nomsg">
          <div class="t_c">
            ...
            <div @click="addSpce" class="sd_btn _mid">
              <img class="_icon" src="@/assets/images/shopSend/optAdd.png" />新增规格
            </div>
          </div>
        </div>
        <!-- flexd button -->
<!--         <div v-if="list.length !== 0" @click="addSpce" class="sd_btn sd_flexd plain">
        ...
        <div v-if="list.length !== 0" class="F">
          ...
          <div @click="closeAll" class="sd_btn">确认选择</div>
        </div>    
        <!-- model -->
        <transition name="fade" >
          <div v-show="showModel" class="sd_model">
            <div  class="sd_model_title">新增规格</div>
            <input class="sd_center" type="text" v-model="spceName" placeholder="请输入规格名称">
